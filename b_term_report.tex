    ...
    \begin{center}
      ...
      \begin{equation}
        \begin{split}
          ...
          p^x_{t+1} = p^x_t + v^x_t\Delta t + a^x_t\Delta t^2 \\
          p^y_{t+1} = p^y_t + v^y_t\Delta t + a^y_t\Delta t^2 \\
        \end{split}
        \label{eq:double_integrate}
      \end{equation}
      Equation \ref{eq:double_integrate}: Double integration of IMU data. $a^x_t$ is the measured acceleration at time t.}
    \end{center}

    We compared the resulting position to the position derived from forward kinematics of the encoder values. Because the Turtlebot is differential drive, moves slowly, and was driven on carpet, there is no slip, and so the position derived from encoders is extremely accurate. Figure \ref{fig:turtlebot_imu} shows how adding a simple bias and scaling factor can improve the accurate of double integration.
